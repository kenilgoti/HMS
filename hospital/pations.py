...
def checkout(request):
    if request.user.category == 'P':
        ...
        room_user_data = bed.objects.filter(user_id=request.user.id)
        check_out_data = {}
        ...
        if request.user.user_bed != None:
            check_out_data['room_bills'] = room_user_data[0].roomId.roomPrice * total_day
        if request.user.medicines != None:
            user_medicine_data = json.loads(request.user.medicines)
        ...
        if 'payment' in request.GET:
            ...
            user_chekout(request, check_out_data)
